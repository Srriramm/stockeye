...
import { BarChart3, MessageSquare, Briefcase, Activity, Bell, TrendingUp, Menu, X, LineChart, AlertTriangle, TrendingDown, Zap, Newspaper, Target, Flame } from 'lucide-react';
    ...
    const fetchGlobal = async () => {
      try {
        const [alertsRes, indicesRes] = await Promise.all([
          axios.get(`${API_URL}/api/alerts?hours=24`),
          ...
          <div className="h-16 flex items-center justify-between px-4 flex-shrink-0" style={{ borderBottom: '1px solid #f1f5f9' }}>
            {sidebarOpen ? (
              <div className="flex items-center gap-3 fade-in">
                ...
                <div>
                  <h1 className="font-bold text-sm tracking-tight" style={{ color: '#0f172a', lineHeight: 1 }}>FinAI</h1>
          ...
          <div className="p-3 flex-shrink-0" style={{ borderTop: '1px solid #f1f5f9' }}>
            <div className="flex items-center gap-2.5 px-1">
              <div
                ...
                style={{ background: connected ? '#10b981' : '#f59e0b', boxShadow: connected ? '0 0 6px rgba(16,185,129,0.5)' : '0 0 6px rgba(245,158,11,0.5)' }}
              />
              {sidebarOpen && (
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-wide" style={{ color: '#0f172a' }}>
                    {connected ? 'Live' : 'Connecting'}
                  </div>
                  <div className="text-[10px] font-mono" style={{ color: '#94a3b8' }}>
                    {connected ? 'Feed active' : 'Reconnecting...'}
                  </div>
                </div>
              )}
            </div>
